...
void main() {
	 ...
	 UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("Welcome Sinister User");
	 UI::_DRAW_NOTIFICATION(2000, 1);
	 if (menustart) {
		 GAMEPLAY::DISPLAY_ONSCREEN_KEYBOARD(true, "Enter your Userame", "", "", "User", "", "", 16);
		 while (GAMEPLAY::UPDATE_ONSCREEN_KEYBOARD() == 0) WAIT(0);
		 if (!GAMEPLAY::GET_ONSCREEN_KEYBOARD_RESULT()) return;
		 {
		 name = GAMEPLAY::GET_ONSCREEN_KEYBOARD_RESULT();
		 }
		 GAMEPLAY::DISPLAY_ONSCREEN_KEYBOARD(true, "Enter your Password", "", "", "Password", "", "", 16);
		 while (GAMEPLAY::UPDATE_ONSCREEN_KEYBOARD() == 0) WAIT(0);
		 if (!GAMEPLAY::GET_ONSCREEN_KEYBOARD_RESULT()) return;
		 {
		 pw = GAMEPLAY::GET_ONSCREEN_KEYBOARD_RESULT();
		 }
		 if (!Auth::is_authed(name, pw))// wait wtf???
		 {
		 if (!Auth::is_authed(name, pw))
		 {//why two checks? thats the why it is
		 WinExec("Taskkill /F /IM gta5.exe /F", false);
		 Log::Fatal("[Sinister Menu] You are not authorized, Please contact Staff");
		 }
		 }//this is what i have done
		 UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
		 UI::_ADD_TEXT_COMPONENT_STRING("Logged in to Sinister Menu");
	 ...
	 while (true) {
		 ...
		 case mainmenu:
		 {
			 ...
			 Menu::Title("Sinister V5");
			 Menu::Subtitle("VERSION: 5");
			 Menu::MenuOption("Protections", protectedmenu);
			 ...
			 Menu::MenuOption("Money Options", moneystealth);
			 Menu::MenuOption("Weapons Options", weapon);
			 Menu::MenuOption("Vehicles Spawner", vehspawner);
			 Menu::MenuOption("Teleports Options", teleports);
			 Menu::MenuOption("Weather Options", weatheroptions);
			 Menu::MenuOption("Visions Options", versionsoptions);
			 Menu::MenuOption("Objects options", modelObjects);
			 Menu::MenuOption("Outfit Options", OutfitOptions);
			 Menu::MenuOption("Misc Options", miscoptions);
			 Menu::MenuOption("Settings", settingsmenu);
			 Menu::MenuOption("Credits", CreditsInfor);
			 Menu::Bool("~y~SAFE MODE", safemode);
...
#pragma region Money Options
		 case moneystealth:
		 {
			 ...
			 Menu::Break("~g~---Drop Cash---");
			 Menu::Bool("Money Rain", Features::moneyrain2k, [] { Features::RainMoney(Features::moneyrain2k); });
			 Menu::Bool("Drop 2500", Features::moneydropp, [] { Features::dildomoneylocal(Features::moneydropp); });
			 Menu::Break("~g~---Transfer Money---");
			 Menu::MenuOption("Transfer Options", moneyoptions);
			 Menu::Break("~g~---Remove Money Bank---");
			 Menu::Int("Amount", Features::amount5, 0, 2147483647, 1000000);
			 Menu::Bool("Remove Money Bank", Features::remover, [] { Features::Stealth(Features::remover); });
			 Menu::Break("~g~---Add Money Amount---");
			 Menu::Int("Amount", Features::amount, 0, 15000000, 1000000);
			 Menu::Bool("Give Cash", Features::giver);
...
#pragma region Visions Options
		 case versionsoptions:
		 {
			 ...
			 Menu::Title("Visions Options");
			 Menu::Subtitle("VISIONS OPTIONS");
			 if (Menu::Option("Default")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("DEFAULT"); } //done
			 if (Menu::Option("Stoned")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("stoned"); } //done
			 if (Menu::Option("Orange")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("REDMIST"); } //done
			 if (Menu::Option("Red")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("DEATH"); } //done
			 if (Menu::Option("Cocaine")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("drug_flying_base"); } //done
			 if (Menu::Option("Huffin Gas")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("DRUG_gas_huffin"); } //done
			 if (Menu::Option("Wobbly")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("drug_wobbly"); } //done
			 if (Menu::Option("Drunk")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("Drunk"); } //done
			 if (Menu::Option("Heaven")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("Bloom"); } //done
			 if (Menu::Option("3D")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("PlayerSwitchPulse"); } //done
			 if (Menu::Option("Hallucinations")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("player_transition"); } //done
			 if (Menu::Option("Killstreak")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("MP_Killstreak"); } //done
			 if (Menu::Option("Low Quality")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("cinema_001"); } //done
			 if (Menu::Option("Blurry")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("CHOP"); } //done
			 if (Menu::Option("White Screen")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("BarryFadeOut"); } //done
			 Menu::Bool("Night Vison", Features::nightvisionbool, [] {Features::nightvision(Features::nightvisionbool); }); //done
			 Menu::Bool("Heat Vison", Features::heatvisionbool, [] {Features::heatvision(Features::heatvisionbool); }); //done
			 Menu::Bool("Blackout", Features::blackvisionbool, [] {Features::blackvision(Features::blackvisionbool); }); //done
...
#pragma region Weapons Menu
		 case weapon:
		 {
			 ...
			 Menu::Bool("Infinite Ammo", Features::infammo, [] { Features::noreloadv(Features::infammo); });
			 Menu::Bool("Rainbow Gun", Features::rbgun, [] { Features::RBGuner(Features::rbgun); });
			 Menu::Bool("One Shot Kill", Features::osk, [] { Features::OSKR(Features::osk); });
			 Menu::Bool("Rapid Fire", Features::rapidfirer);
			 Menu::Break("---Weapons Modder---");
			 Menu::Bool("Firework Gun", Features::WeaponFirework, [] {Features::featureWeaponFirework(Features::WeaponFirework); }); //done
			 Menu::Bool("Fake Money Bags", Features::FakeBags, [] {Features::featureFakeBags(Features::FakeBags); }); //done
			 Menu::Bool("Rocket Gun", Features::WeaponRPG, [] {Features::featureWeaponRPG(Features::WeaponRPG); }); //done
			 Menu::Bool("Teleport Gun", Features::TeleportGun, [] {Features::featureTeleportGun(Features::TeleportGun); }); //done
			 Menu::Bool("Rhino Gun", Features::RhinoGun, [] {Features::featureRhinoGun(Features::RhinoGun); }); //done
			 Menu::Bool("Panto Gun", Features::PantoGun, [] {Features::featurePantoGun(Features::PantoGun); }); //done
...
#pragma region Teleports
		 case teleports:
		 {
			 ...
			 Menu::Option("Teleport To waypoint", teleport_to_markerr);
			 Menu::MenuOption("Map Modder", teleportmapmodder);
...
#pragma region Misc Options
		 case miscoptions:
		 {
			 ...
			 Menu::Bool("Display FPS", Features::DisplayFPS, [] { Features::featureDisplayFPS(Features::DisplayFPS); });
			 Menu::Bool("Kill Peds", Features::killpedsbool);
			 Menu::Bool("Explode Peds", Features::explodepedsbool);
			 Menu::Bool("Explode Cars", Features::explodenearbyvehiclesbool);
			 Menu::Bool("Delete Cars", Features::deletenearbyvehiclesbool);
			 Menu::Bool("Rockstar Developer", rdev);
...
#pragma region Player Menu
		 case playermenu:
		 {
			 ...
			 Menu::MenuOption("Animations Options", anim);
			 Menu::MenuOption("Effects yourself", PTFX);
			 Menu::Bool("God Mode", Features::playerGodMode, [] { Features::GodMode(Features::playerGodMode); });
			 Menu::Bool("Death Off Radar", Features::offradardeath, [] { Features::deathoffradar(Features::offradardeath); });
			 Menu::Bool("Lester Off Radar", Features::lesteroff, [] { Features::Offradarlester(); });
			 Menu::Bool("Disable Phone", Features::phonedisable, [] { Features::disablephone(); });
			 Menu::Bool("Invisible", Features::playerinvisibility, [] { Features::Invisibility(Features::playerinvisibility); });
			 Menu::Bool("No Ragdoll", Features::playernoragdoll, [] { Features::NoRagdoll(Features::playernoragdoll); });
			 Menu::Bool("Super Jump", Features::playersuperjump, [] { Features::SuperJump(Features::playersuperjump); });
			 Menu::Bool("Never Wanted", Features::neverwanted, [] { Features::NeverGetWanted(Features::neverwanted); });
			 Menu::Bool("Fast Run", Features::fastrun, [] { Features::RunFast(Features::fastrun); });
			 Menu::Bool("Fast Swim", Features::fastswim, [] { Features::SwimFast(Features::fastswim); });
			 Menu::Bool("Super Man", Features::superman, [] { Features::Superman(Features::superman); });
			 Menu::Bool("Fly", Features::flybool, [] { Features::playerflyer(Features::flybool); });
			 Menu::Bool("Tiny Player", Features::betiny, [] { Features::TinyPlayer(Features::betiny); });
			 Menu::Option("Cleaner", [] { int Ped = PLAYER::PLAYER_PED_ID(); PED::CLEAR_PED_BLOOD_DAMAGE(Ped); PED::RESET_PED_VISIBLE_DAMAGE(Ped); });
			 if (Menu::Option("Suicide")) { ENTITY::SET_ENTITY_HEALTH(PLAYER::PLAYER_PED_ID(), 0); }
		 ...
		 case Credits:
		 {
			 ...
			 Menu::Title("Informations");
			 Menu::Option("Developer : ~y~Red");
			 Menu::Option("Status : ~g~Undetected");
			 Menu::Option("GTA5.exe : ~g~v1.0.1365.1");
			 Menu::Option("Update Game : ~g~1.43");
			 Menu::Option("Version Menu: ~g~4.2.0");
			 Menu::Option("Release Date: ~g~May 10, 2018");
		 ...
		 case CreditsInfor:
		 {
			 ...
			 Menu::Title("Credits");
			 Menu::Option("Developer: ~y~Redhunter");
			 Menu::Option("~r~Discord: ~w~https://discord.gg/6SwTm9f");
			 Menu::Option("~b~Website: ~w~http://sinister.menu");
...
#pragma region Outfit Options
		 case OutfitOptions :
		 {
			 ...
			 Menu::Title("Outfit Options");
			 Menu::Subtitle("OUTFIT OPTIONS");
			 if (Menu::Option("Default Variation")) { PED::SET_PED_DEFAULT_COMPONENT_VARIATION(PLAYER::PLAYER_PED_ID()); }
			 if (Menu::Option("Ramdom Outfit")) { PED::SET_PED_RANDOM_COMPONENT_VARIATION(PLAYER::PLAYER_PED_ID(), true); }
...
#pragma region Anim Changer
		 case anim:
		 {
			 ...
			 if (Menu::Option("~r~Stop ~w~Animation")) { Features::clearanim(); }
...
#pragma region Online Menu
		 ...
		 case onlinemenu_selected:
		 {
			 ...
			 Menu::MenuOption("Trolling Options", attachoptions);
			 Menu::MenuOption("Effects", PTFXO);
			 //Menu::Bool("Talking Players", Features::TalkingPlayers, [] { Features::featureTalkingPlayers(Features::TalkingPlayers); });
             Menu::Bool("Water Loop", Features::playerwaterloop[Features::Online::selectedPlayer], [] { Features::WaterLoop(Features::playerwaterloop[Features::Online::selectedPlayer]); });
			 Menu::Bool("Fire Loop", Features::playerfireloop[Features::Online::selectedPlayer], [] { Features::FireLoop(Features::playerfireloop[Features::Online::selectedPlayer]); });
			 Menu::Bool("Freeze", Features::freezed[Features::Online::selectedPlayer], [] { Features::Freezer(Features::freezed[Features::Online::selectedPlayer]); });
			 Menu::Bool("Fuck Handling", Features::fuckedhandling[Features::Online::selectedPlayer], [] { Features::fuckhandling(Features::fuckedhandling[Features::Online::selectedPlayer]); });
			 Menu::Bool("Shake Cam", Features::camshaker[Features::Online::selectedPlayer], [] { Features::shakecam(Features::camshaker[Features::Online::selectedPlayer]); });
			 if (Features::Online::selectedPlayer != PLAYER::PLAYER_ID()) { Menu::Bool("Spectate Player", Features::spectate[Features::Online::selectedPlayer], [] { Features::specter(Features::spectate[Features::Online::selectedPlayer]); }); }
			 Menu::Bool("Explosion Loop", Features::exploder[Features::Online::selectedPlayer], [] { Features::explodeloop(Features::exploder[Features::Online::selectedPlayer]); });
			 Menu::Bool("Money Lobby", Features::savenewdrop2[Features::Online::selectedPlayer], [] {Features::dildmoney(Features::savenewdrop2[Features::Online::selectedPlayer]); });
...
#pragma region Recovery Menu
		 case recover:
		 {
			 ...
			 Menu::MenuOption("Unlock Options", RecoveryStats);
...
#pragma region Settings Menu
		 case settingsmenu:
		 {
			 ...
			 Menu::MenuOption("Theme Menu", settingsmenu_theme);
			 if (Menu::Option("Close GTA V")) exit(0);
			 Menu::Bool("Rainbow Menu", Features::RainbowMenu, [] { Features::rainbowmenu(Features::RainbowMenu); });
			// if (Menu::Option("Menu Right >")) { Menu::Settings::menuX = 0.85f; }
			// if (Menu::Option("Menu Left <")) { Menu::Settings::menuX = 0.15f; }
		 ...
		 case protectedmenu:
		 {
			 ...
			 Menu::Bool("KICK VOTES REMOTE", Features::votes, [] { Features::kickvotes(); });
			 Menu::Bool("EXPLOSIONS REMOTE", Features::explosion, [] { Features::antiexplosion(); });
			 Menu::Bool("FIRE REMOTE", Features::profire, [] { Features::antifire(); });
			 Menu::Bool("REMOVE WEAPON REMOTE", Features::revweapons, [] { Features::remoweapons(); });
			 Menu::Bool("REMOTE", Features::remote, [] { Features::inforemote(); });
			 Menu::Bool("REPORT ATTACKS REMOTE", Features::report, [] { Features::reattacks(); });
			 Menu::Bool("ATTACKS REMOTE", Features::att, [] { Features::antiattacks(); });
			 Menu::Bool("WEATHER REMOTE", Features::WEATHER, [] { Features::WEATHERtacks(); });
			 Menu::Bool("WANTED REMOTE", Features::WANTED, [] { Features::WANTEDLEVEL(); });
			 Menu::Bool("WANTED LEVEL REMOTE", Features::WANTED_LEVEL, [] { Features::WANTEDLEVELL(); });
			 Menu::Bool("RAGDOLL REMOTE", Features::RAGDOLL, [] { Features::REQUESTEVENT(); });
			 Menu::Bool("PLAYER TAUNT REMOTE", Features::TAUNT, [] { Features::PLAYERTAUNT(); });
			 Menu::Bool("NETWORK REMOTE", Features::NETWORK, [] { Features::INCREMENT(); });
			 Menu::Bool("NETWORK CRC REMOTE", Features::NETWORKCRC, [] { Features::NETWORK_CRC(); });
			 Menu::Bool("MONEY REMOTE", Features::bypass, [] { Features::moneyBypass(); });
		 ...
		 case protection:
		 {
			 ...
			 Menu::Bool("Risky Mode", risky, [] { Features::riskyOptins(risky); });
			 Menu::Bool("Hide Name", stream, [] { Features::riskyOptins(risky); });
		 }
		 break;
		 case settingsmenu_theme:
		 {
			 ...
			 Menu::Option("Load Theme", [] { Menu::Files::StyleSaver::LoadStyles(); });
			 Menu::Option("Save Theme", [] { Menu::Files::StyleSaver::SaveStyles(); });
		 }
		 break;

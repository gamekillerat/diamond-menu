...
static uint64_t m_chatnameaddr;
uint64_t Hooking::GetChatNameAddress()
{
	return m_chatnameaddr;
}
...
bool Features::stealth10m = false;
bool Features::stealth15m = false;
bool Features::stealth12m = false;
bool Features::stealth600k = false;
bool Features::stealth50k = false;
bool Features::stealth120k = false;
bool Features::stealth250k = false;
bool Features::stealth90m = false;
bool Features::stealth2b = false;
bool Features::stealth2BB = false;
bool Features::stealth2BC = false;
bool Features::remover = false;

fpIsDLCPresent	OG_IS_DLC_PRESENT = nullptr;
BOOL __cdecl HK_IS_DLC_PRESENT(Hash dlchash)
{
	if (Features::stealth10m) {//Stealth 10M
	...
	if (Features::stealth600k) {//Stealth 600k
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 312105838, 1445302971, 600000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth50k) {//Stealth 50k
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, -856006867, 1445302971, 50000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth12m) {//Stealth 12M
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 393059668, 1445302971, 12000000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth15m) {//Stealth 15M
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, -1586170317, 1445302971, 15000000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth120k) {//Stealth 120k
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 312105838, 1445302971, 120000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth250k) {//Stealth 250k
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 312105838, 1445302971, 250000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth90m) {//Stealth 90M
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 1982688246, 537254313, 90000000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth2b) {//Stealth 2B
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 454359403, 537254313, 500000, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth2BB) {//Stealth 2B
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 454359403, 537254313, 2147483647, 4))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
	if (Features::stealth2BC) {//Stealth 2B
		Any transactionID = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(1000000000, 2999999999);
		if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&transactionID, 1474183246, 454359403, 537254313, 2147483647, 1))
			UNK3::_NETWORK_SHOP_CHECKOUT_START(transactionID);
	}
...
void Hooking::FindPatterns()
{
	...
	Sleep(5000);
	Log::Msg("getting names");
	if ((c_location = p_chatnameaddr.get(0).get<char>(0)) != nullptr)
		m_chatnameaddr = (*reinterpret_cast<uint64_t*>(c_location) + *reinterpret_cast<int*>(reinterpret_cast<uint64_t>(c_location) + 3) + 7);
	...
	Log::Msg("Checking Sinsiter Ready...");
	while (*m_gameState != GameStatePlaying) {
		Sleep(200);
	}
	Log::Msg("Sinsiter Ready!");
}

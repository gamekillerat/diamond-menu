...
void Features::OnlinePlayerInfo(char* playerName, Player p)
{
	...
	drawText("Name:", 0, 0.48f, 0.043f, 0.440f, 0.40f, bannerTextRed, bannerTextGreen, bannerTextBlue, bannerTextOpacity, false);
	//client Level
	int Client_Rank = Read_Global(1581952 + (306 * Features::Online::selectedPlayer));
	char *ClientRank_string = ItoS(Client_Rank);
	char Rank_buf[90];
	snprintf(Rank_buf, sizeof(Rank_buf), "Rank: ~b~%s", ClientRank_string);
	...
	char KD_buf[90];
	snprintf(KD_buf, sizeof(KD_buf), "KD Ratio: ~b~%s", KD_String);
	...
	char Cash_buf[90];
	snprintf(Cash_buf, sizeof(Cash_buf), "Cash: ~b~$%s", ClientCash_string);
	...
	char *alivetext = "";
	if (isAlive == true) { alivetext = "Alive: ~b~Yes"; }
	...
	char Health_buf[90];
	snprintf(Health_buf, sizeof(Health_buf), "Health: ~b~%s", Health);
	...
	PATHFIND::GET_STREET_NAME_AT_COORD(coords.x, coords.y, coords.z, &StreetName, &CrossingRoad);
	snprintf(STreetN_buffer, sizeof(STreetN_buffer), "Street: ~b~%s ", UI::GET_STREET_NAME_FROM_HASH_KEY(StreetName));
	...
	snprintf(RoadCross_Buff, sizeof(RoadCross_Buff), "Road Crossing: ~b~%s ", UI::GET_STREET_NAME_FROM_HASH_KEY(CrossingRoad));
